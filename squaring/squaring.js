    ...
    for(var i=1;i<=4;i++)
        {
            if(i==t)
                ...
                document.getElementById('btn'+i).innerHTML=res - (Math.floor(Math.random() * 10) * 10);
            else
                document.getElementById('btn'+i).innerHTML=res + (Math.floor(Math.random() * 10) * 20);
        }
}
function check_mul(res,l,s,e,c,p,a){
...
function squ(l,s,e,c,p){
    var a = Math.floor(Math.random() * e)+s;
    var res = a * a;
    $('.qs').text((c+1)+" - 12");
    display_mul(a,res);
    check_mul(res,l,s,e,c,p,a);
}
